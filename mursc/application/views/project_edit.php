                ...
                <fieldset class="col-lg-offset-1">
                    <div class="col-lg-11">
                        <?php
                            ...
                            {
                                $this->table->add_row($m['username'],
                                    form_dropdown('status_member_' . $m['user_id'], $status_array, $m['status_id']),
                                    '<label>' . form_checkbox('dismiss_member_' . $m['user_id'], 1,  FALSE) . ' dismiss</label>');
                            }
                            $this->table->add_row(array('data' => 'Candidates', 'colspan' => 3, 'style' => 'font-weight : bold; text-align : center;'));
                            foreach ($list_candidate as $c)
                            {
                                $this->table->add_row($c['username'],
                                    form_dropdown('status_candidate_' . $c['user_id'], $status_array, ''),
                                    '<label>' . form_radio("candidature_" . $c['user_id'], 'accept', FALSE)  . ' accept </label>' . ' ' . '<label>' . form_radio("candidature_" . $c['user_id'], 'reject', FALSE)  . ' reject </label>' . ' ' . '<label>' . form_radio("candidature_" . $c['user_id'], 'none', TRUE)  . ' none </label>');
                            }
                            $this->table->add_row(array('data' => 'Invitated', 'colspan' => 3, 'style' => 'font-weight : bold; text-align : center;'));
                            foreach ($list_invited as $i)
                            {
                                $this->table->add_row($i['username'],
                                    form_dropdown('status_invited_' . $i['user_id'], $status_array, $i['status_id']),
                                    '<label>' . form_checkbox('close_invitation_' . $i['user_id'], 1,  FALSE) . ' cancel</label>');
                            }
